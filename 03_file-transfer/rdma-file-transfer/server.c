...
static void on_completion(struct ibv_wc *wc)
{
  ...
  if (wc->opcode == IBV_WC_RECV_RDMA_WITH_IMM) {
    ...
    if (size == 0) {
      ...
      uint64_t now = get_clocktime();
      uint64_t diff = (now - ctx->start_time)/BILLION;
      double tput = (ctx->total_bytes_received/1000000)/diff;
      printf("total bytes %lu MB, total_time %lu s, tput: %f MB/s\n", ctx->total_bytes_received/1000000, diff, tput);
      ...
      ctx->total_bytes_received += size;
      int index = ctx->chunk_num%2;
      ctx->chunk_num++;
      ret = write(ctx->fd, ctx->buffer+(BUFFER_SIZE/2)*index, size);
